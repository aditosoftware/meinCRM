...
//Beleg kopieren
var head = a.sql("select CURRENCY, LANGUAGE, PAYMENTTERMS, REMARK, RELATION_ID, SALESORDERID, PROJECT_ID, PROPERTY_ID, ADDRESS_INVOICE "
    + " FROM SALESORDER WHERE SALESORDERID = '" + id + "'", a.SQL_ROW);
//Rechnung auf 'storniert' setzen
a.sqlUpdate("SALESORDER", ["CANCELLED"], [SQLTYPES.CHAR], ["Y"], " SALESORDERID = '" + id + "'")
// Posten kopieren
...
var item = a.sql("select ORDERITEM.GROUPCODEID, PRODUCTID, PRODUCTNAME, PRODUCTCODE, ORDERITEM.UNIT, PRICE, QUANTITY, "
    + " DISCOUNT, QUANTITY * PRICE * (100 - DISCOUNT) / 100, VAT, ORDERITEM.DESCRIPTION"
...
defaultvalue["$comp.ADDRESS_INVOICE"] = head[8];
defaultvalue["$comp.ORDERTYPE"] = "4";
